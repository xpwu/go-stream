...
func PushDataToClient(ctx context.Context, clientCon conn.Conn, data []byte) State {
	...
	if uint64(len(data)) > uint64(clientCon.ServerConfig().MaxBytesPerFrame) {
		logger.Error(fmt.Sprintf("data(len=%d) is too large, which can not be pushed to client connection(conn_id=%s)",
			len(data), clientCon.Id()))
